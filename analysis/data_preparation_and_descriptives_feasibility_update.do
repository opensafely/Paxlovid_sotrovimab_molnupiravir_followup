...
* Open a log file
cap log close
log using ./logs/data_preparation_feasibility_update, replace t
clear

* import dataset
import delimited ./output/input_feasibility_update.csv, delimiter(comma) varnames(1) case(preserve) 
*describe
...
gen infect_month=year(covid_test_positive_date)*100+month(covid_test_positive_date)
tab infect_month
gen infect_month2=year(covid_test_positive_date2)*100+month(covid_test_positive_date2)
tab infect_month2
tab region_nhs,m
...
gen sotro_month_o=year(sotrovimab_covid_therapeutics_o)*100+month(sotrovimab_covid_therapeutics_o)
gen pax_month_o= year(paxlovid_covid_therapeutics_o)*100+month(paxlovid_covid_therapeutics_o)
gen mol_month_o= year(molnupiravir_covid_therapeutics_)*100+month(molnupiravir_covid_therapeutics_)
gen rem_month_o= year(remdesivir_covid_therapeutics_o)*100+month(remdesivir_covid_therapeutics_o)
gen treated_month_o= year(date_treated_out)*100+month(date_treated_out)
tab sotro_month_o
tab pax_month_o
tab mol_month_o
tab rem_month_o
...
gen sotro_month_GP= year(sotrovimab_GP)*100+month(sotrovimab_GP)
gen pax_month_GP= year(paxlovid_GP)*100+month(paxlovid_GP)
gen mol_month_GP= year(molnupiravir_GP)*100+month(molnupiravir_GP)
gen rem_month_GP= year(remdesivir_GP)*100+month(remdesivir_GP)
gen treated_month_GP= year(date_treated_GP)*100+month(date_treated_GP)
tab sotro_month_GP
tab pax_month_GP
tab mol_month_GP
tab rem_month_GP
...
gen treated_month= year(date_treated)*100+month(date_treated)
tab treated_month
tab region_nhs treated_month, row
tab stp treated_month,row

gen treated_month_hosp= year(date_treated_hosp)*100+month(date_treated_hosp)
...
gen treated_month_onset= year(date_treated)*100+month(date_treated)
